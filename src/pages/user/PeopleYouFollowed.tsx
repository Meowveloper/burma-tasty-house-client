...
export default function PeopleYouFollowed() {
    ...
    return (
        <div>
            <div className="text-h2 font-bold mb-4">People You Followed</div>
            {loading ? (
                <div>Loading.....</div>
            ) : !users.length ? (
                <div className="col-span-2 tablet:col-span-3 desktop:col-span-4">No users found..., you haven't followed any users</div>
            ) : (
...
function getFetchUsersFunction(setUsers: React.Dispatch<React.SetStateAction<IUser[]>>, setLoading: React.Dispatch<React.SetStateAction<boolean>>) {
    return async function () {
        try {
            setLoading(true);
            const result = await axios.get("/users/get-people-you-follow");
            console.log(result);
